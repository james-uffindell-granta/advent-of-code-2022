...
pub fn input_generator_part1(input: &str) -> Input {
    ...
    for l in input.lines() {
        ...
        for (idx, _) in crates {
            ...
            dock.entry(stack_number).or_insert(Vec::new()).insert(0, _crate);
        }
        ...
        if l.starts_with("move") {
            ...
            instructions.push(Instruction { number_to_move: number_to_move, start_stack: starting_stack, end_stack: ending_stack })
        }
    }

    Input { starting_layout: dock, instructions: instructions }
}
...
pub fn solve_part1(input: &Input) -> String {
    ...
    (1..=number_of_stacks).map(|i| dock.get_mut(&i).unwrap().pop().unwrap()).collect()
}
...
"#;
    ...
    let expected = Input {
        starting_layout: HashMap::from([(1, vec!['Z', 'N']), (2, vec!['M', 'C', 'D']), (3, vec!['P'])]),
        instructions: vec![
            Instruction { number_to_move: 1, start_stack: 2, end_stack: 1 },
            Instruction { number_to_move: 3, start_stack: 1, end_stack: 3 },
            Instruction { number_to_move: 2, start_stack: 2, end_stack: 1 },
            Instruction { number_to_move: 1, start_stack: 1, end_stack: 2 },
        ]
    };
