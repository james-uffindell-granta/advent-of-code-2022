...
pub struct Tree { height: u32 }

pub struct Grid { trees: Vec<Vec<Tree>> }

impl Grid {
    pub fn row_count(&self) -> usize { self.trees.len() }
    pub fn column_count(&self) -> usize { self.trees[0].len() }
...
fn is_tree_visible((row, column): (&Vec<Tree>, &Vec<Tree>), (row_index, column_index): (usize, usize)) -> bool {
...
where I : IntoIterator<Item=&'a Tree> {
...
fn scenic_score((row, column): (&Vec<Tree>, &Vec<Tree>), (row_index, column_index): (usize, usize)) -> u32 {
    let tree_height = row[row_index].height;
    let (left, tree_onwards_right) = row.split_at(row_index);
    let (up, tree_onwards_down) = column.split_at(column_index);
...
pub fn input_generator_part1(input: &str) -> Grid {
    Grid { trees: input.lines()
        .map(|l| l.chars()
            .map(|c| c.to_digit(10).unwrap()).map(|i| Tree { height: i } )
            .collect())
        .collect() }
}
...
pub fn solve_part1(input: &Grid) -> usize {
    (0..input.column_count()).map(|r| {
        let column = input.column(r);
        (0..input.row_count()).filter(|&c| {
            is_tree_visible((input.row(c), &column), (r, c))
        }).count()
    }).sum()
}
...
pub fn solve_part2(input: &Grid) -> u32 {
    (0..input.column_count()).map(|r| {
        let column = input.column(r);
        (0..input.row_count()).map(|c| {
            scenic_score((input.row(c), &column), (r, c))
        }).max().unwrap()
    }).max().unwrap()
}
