...
pub fn input_generator_part1(input: &str) -> Vec<AssignmentPair> {
    ...
    for l in input.lines() {
        ...
        pairs.push(AssignmentPair {
            first: SectionRange { start_id: first_start.parse::<u32>().unwrap(), end_id: first_end.parse::<u32>().unwrap() },
            second: SectionRange { start_id: second_start.parse::<u32>().unwrap(), end_id: second_end.parse::<u32>().unwrap() },
        });
    }
...
pub fn solve_part1(input: &[AssignmentPair]) -> usize {
    input.iter().filter(|p| p.first.fully_contains(&p.second) || p.second.fully_contains(&p.first)).count()
}
