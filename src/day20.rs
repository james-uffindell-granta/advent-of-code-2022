...
impl Entry {
    pub fn with_value(v: i64, idx: usize) -> Self {
        Self { value: v, original_index: idx, }
    }
}
...
pub fn shuffle_vector_by_original_index(input: &mut Vec<Entry>) {
    ...
    for idx in 0..number {
        if let Some(index) = input.iter().position(|e| e.original_index == idx) {           
            ...
            if amount_to_move == 0 {
                // nowhere to actually move it
                continue;
            } else if amount_to_move > 0 {
...
#[aoc(day20, part1)]
pub fn solve_part1(input: &Vec<Entry>) -> i64 {
...
#[aoc(day20, part2)]
pub fn solve_part2(input: &Vec<Entry>) -> i64 {
    let mut input = input.iter().map(|e| {
        Entry {
